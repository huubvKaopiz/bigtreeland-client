...
function LeftMenu(): JSX.Element {
	...
	useEffect(() => {
		...
		if (
			...
			pathname.includes("/user-set-permissions/")
		) {
			setOpenKeys([""])
			setSelectedKeys(["2"])
			return
		...
		if (
			...
			pathname.includes("/salaries-create")
		) {
			setOpenKeys(["employees"])
			setSelectedKeys(["4"])
			return
		...
		if (
			...
			(pathname.includes("/study") && !pathname.includes("/study-summary"))
		) {
			setOpenKeys(["classes"])
			setSelectedKeys(["5"])
			return
		...
		if (
			...
			pathname.includes("/students-study-profile")
		) {
			setOpenKeys(["classes"])
			setSelectedKeys(["6"])
			return
		...
		if (
			pathname.includes("/study-summary") ||
			pathname.includes("/study-summary-detail")
		) {
			setOpenKeys(["classes"])
			setSelectedKeys(["8"])
			return
