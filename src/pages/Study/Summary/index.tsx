...
export function StudySumaryBoard(props: { class_id?: number }): JSX.Element {
	...
	const classList = useSelector(
		(state: RootState) => state.classReducer.classes
	)
	const studySummaryList = useSelector(
		(state: RootState) => state.studySummaryReducer.studySummaryList
	)
	const getStudySummaryListStatus = useSelector(
		(state: RootState) => state.studySummaryReducer.getStudySummaryListState
	)
	...
	useEffect(() => {
		if (class_id) {
			...
			dispatch(actionGetClasses({ per_page: 100000 }))
		}
	}, [dispatch, class_id])
		...
		<Layout.Content>
			{!class_id && (
				<Space style={{ marginBottom: 20, marginTop: 20 }}>
					...
					{(isAdmin ||
						isHavePermission(permissionList, "study-summary-boards.store")) && (
						<CreateStudySummary
							classList={get(classList, "data", [])}
							class_id={0}
						/>
					)}
				</Space>
						...
						actions={[
							(isAdmin ||
								...
								<Button
									...
									onClick={() =>
										history.push({
											pathname: `/study-summary-detail/${item.id}`,
											state: { summaryInfo: item },
										})
									}
								>
									Chi tiết
					...
					>
						<List.Item.Meta
							...
							title={
								...
								>
									{`${moment(item.from_date).format("DD/MM/YYYY")} - ${moment(
										item.to_date
									).format("DD/MM/YYYY")}`}
								</a>
							}
