...
export default function (props: {
	...
	const cols = [
		...
		},
		{
			...
			render: function col(val: string, record: StudentType): JSX.Element {
				return (
					<Button
						type="link"
						onClick={() => history.push(`/students-study-profile/${record.id}`)}
					>
						{val}
					</Button>
				);
		...
		},
		{
			...
			dataIndex: "acrion",
			key: "action",
            hidden: !(isAdmin || isHavePermission(permissionList, "classes.update")),
			render: function col(_: string, record: StudentType): JSX.Element {
				return (
					<Space>
	...
	return (
		<div>
			{
                // Phải có quyền thay đổi thông tin lớp học mới thêm học sinh vào lớp
			...
			<Table
				bordered
				style={{ marginTop: 20 }}
				columns={cols.filter((item) => !item.hidden)}
				dataSource={students}
				pagination={{
					pageSize: 30,
				}}
