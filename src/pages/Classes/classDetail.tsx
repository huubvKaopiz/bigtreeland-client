import { Layout, PageHeader, Tabs, Button, DatePicker, Descriptions, Table, Space, List, Image, Input } from 'antd';
...
export default function ClassDetail(): JSX.Element {
    ...
    const todayCol = {
        title: <><Checkbox onChange={handleCheckAll} /> {today} </>,
        key: 'operation',
        dataIndex: "",
        width: 40,
        fixed: false,
    ...
    const commentCol = {
        ...
        render: function col(st: { id: number }): JSX.Element {
            return (
                <Space> <Input placeholder="Nhận xét" onChange={(e) => handleChangeComment(e, st.id)} /></Space>
            );
        },
    }
