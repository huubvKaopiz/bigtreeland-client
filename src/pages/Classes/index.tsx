...
function Classes(): JSX.Element {
	...
	useEffect(() => {
		...
		dispatch(actionGetEmployees({ role_id:`2,3` })); //role_id of teacher and teacher2
	}, [dispatch]);
	...
	const searchTeacher = (search: string) => {
		if (search.length >= 3 || search.length === 0) dispatch(actionGetEmployees({role_id:`2,3`, search }));
	};
