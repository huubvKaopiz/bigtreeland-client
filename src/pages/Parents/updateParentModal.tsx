...
export default function UpdateParent(props: { parent: ParentType }): JSX.Element {
    ...
    useEffect(() => {
        if (parent) {
            uForm.setFieldsValue({
                name: parent.name,
                phone: parent.user && parent.user.phone,
                email: parent.email
            })
        }
    },[parent, uForm])
    ...
    return (
        <div>
            <Tooltip placement="top" title="Sửa thông tin"> <Button icon={<EditOutlined />} type="link" onClick={() => setShow(true)} /></Tooltip>
            <Modal
                ...
                width={800}
                footer={[
                    <Button key="btncanl" onClick={() => setShow(false)}>Huỷ bỏ</Button>,
                    <Button key="btnSubmit" htmlType="submit" form="aForm" type="primary">Lưu lại</Button>,
                ]}
            >
                <Form
                    id="aForm"
