    ...
    const columns = [
...

        {
            ...
            render: function UserCol(user: { id: number, phone: string, phone_verified_at: string }): JSX.Element {
                return (
                <Space>{user.phone} {user.phone_verified_at == null ? <Tag color="volcano">Chưa xác thực</Tag> : ""}</Space>
                ...
                return (
                    <Space>
                       <DeleteParent  parent={record}/>
                        {record.user.phone_verified_at == null ? <Tooltip placement="top" title="Xác minh"><Button type="link" icon={<CheckCircleOutlined />}/></Tooltip> : "" }
