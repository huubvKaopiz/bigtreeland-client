...
function Payment(): JSX.Element {
	...
	useEffect(() => {
		if (
			statusGetPayment === "loading" ||
			statusUpdatePayment === "loading" ||
			statusDeletePayment === "loading"
		)
			setLoading(true);
	...
	const tableColumn = [
		...
		},
		{
			...
			render: function NameCol(_: string, row: any): JSX.Element {
				return <a>{get(row, "payer.profile.name", "")}</a>;
			},
		},
		{
			width: "10%",
				...
				{selectedPaymentRows.length > 0 && (
					<Space style={{ marginBottom: 20 }}>
						<Button
							...
							onClick={() => handleMultipleConfirmed()}
							loading={false}
						>
							Xác nhận
						</Button>
						<span style={{ marginLeft: 8 }}>
							{selectedPaymentRows.length > 0
								? `${selectedPaymentRows.length} phiếu chi`
								: ""}
						</span>
					</Space>
				)}
